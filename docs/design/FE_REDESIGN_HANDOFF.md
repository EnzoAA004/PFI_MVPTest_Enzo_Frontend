...
| 4 | FE-RD-004 | Landmarks y mediciones editables | APROBADA Y CERRADA (Reset to AI cableado; mediciones persisten vía PUT .../measurements; landmarks move/add gated por coordinateSpace; versionado marcado pendiente). Fix aplicado en Fase 5 Parte A: landmarks fuera del campo de notas → borrador local "no persistido". |
...
| 7 | FE-RD-007 | Limpieza de bloques legacy (deuda) | APROBADA Y CERRADA (bloques legacy-* eliminados; fix aplicado: reglas .unified-results-table/.compact-comparison-row/.ai-completion-table restauradas; verificado 197 clases usadas / 0 sin regla; build 1834; 0 hex; smoke Settings OK). |
