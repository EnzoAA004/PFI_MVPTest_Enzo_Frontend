...
const display = load("src/features/reading/discFindingDisplay.ts", [
  "DISC_FINDING_LABELS", "DEPLOYMENT_LABELS", "DEPLOYMENT_NOTES", "showsProbabilities",
  "startsCollapsed", "DEPLOYMENT_ORDER", "DISC_FINDINGS_NOTICE",
]);
...
check("las tareas de investigación no muestran barra de probabilidad", () => {
  // Un 88% con F1 0,125 se dibuja idéntico a un 88% con F1 0,846, y al lado de una imagen
  // del paciente los dos se leen igual de convincentes.
...
check("la nota explica por qué no hay números, y no habla de métricas", () => {
  // Sin la explicación, la ausencia se lee como un dato que falta en vez de una decisión.
  // Y "F1 0,125" no le dice nada a quien lee un estudio.
  const nota = display.DEPLOYMENT_NOTES.not_product_supported;
  assert.ok(nota.includes("minoría de los casos"));
  assert.ok(nota.includes("No se muestran probabilidades"));
