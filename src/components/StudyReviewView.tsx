...
export function StudyReviewView({ run, studyReview, measurements, auditTrail, saving, onMeasurementsChange, onSaveReview }: StudyReviewViewProps) {
  ...
  const studyMeasurements = Array.isArray(studyReview?.measurements) && studyReview.measurements.length
    ? studyReview.measurements
    : sourceMeasurements.map((item) => ({ id: item.id, label: item.label, level: item.label.includes("L5-S1") ? "L5-S1" : "L4-L5", value: Number(item.value) || 0, aiValue: Number(item.value) || 0, reviewerValue: null, unit: item.unit, confidence: item.confidence ?? 0.72, status: item.status, outlier: Boolean(item.outlier), linkedLandmarks: [] }));
  ...
  function toggleMask(maskId: string) { setMaskVisibility((current) => ({ ...current, [maskId]: !(current[maskId] ?? true) })); }
  function selectSeries(series: any) { setSelectedSeriesId(series.id); setTab(series.plane === "axial" ? "Axial" : "Sagittal"); }
  function updateReviewerValue(measurement: any, value: string) { setReviewerValues((current) => ({ ...current, [measurement.id]: value })); }
  function getPersistedReviewerValue(measurementId: string) { const persisted = sourceMeasurements.find((item) => item.id === measurementId && item.source === "Reviewer"); return persisted?.value ?? ""; }
  ...
  function toMeasurement(item: any): Measurement {
    ...
    return { id: item.id, label: item.label, value: reviewerValue ?? existing?.value ?? item.value ?? item.aiValue ?? "", unit: item.unit ?? existing?.unit ?? "", confidence: item.confidence ?? existing?.confidence, plane: existing?.plane ?? displayRun.plane, source: reviewerValue !== undefined && reviewerValue !== "" ? "Reviewer" : existing?.source ?? "AI", status: reviewerValue !== undefined && reviewerValue !== "" ? "editado" : existing?.status ?? "pendiente", outlier: Boolean(item.outlier ?? existing?.outlier), placeholder: existing?.placeholder };
  }
  ...
  function commitReviewerMeasurements() {
    ...
    const appended = studyMeasurements.filter((item: any) => reviewerValues[item.id] !== undefined && reviewerValues[item.id] !== "" && !existingIds.has(item.id)).map(toMeasurement);
    onMeasurementsChange([...updated, ...appended], `${reviewerDraftCount} medicion/es guardadas por Reviewer desde Resultados IA`);
    setReviewerValues({});
  }

  async function save(status: ReviewStatus) { setReviewStatus(status); if (hasReviewerDrafts) commitReviewerMeasurements(); await onSaveReview(status, notes); }
  function panelVisible(panelId: string) { return !hiddenPanels[panelId]; }
  function togglePanel(panelId: string) { setHiddenPanels((current) => ({ ...current, [panelId]: !current[panelId] })); }

  function PanelTitle({ panelId, title, children }: { panelId: string; title: string; children?: any }) {
    const visible = panelVisible(panelId);
    return <div className="section-title"><h2>{title}</h2><div className="panel-title-actions">{children}<button className="visibility-toggle" onClick={() => togglePanel(panelId)} type="button" aria-label={visible ? `Ocultar ${title}` : `Mostrar ${title}`}><VisibilityIcon visible={visible} /></button></div></div>;
  }
    ...
    <div className="view-stack review-workspace clinical-quiet">
      ...
      <section className="review-grid">
        <aside className="left-column">
          <article className="panel-card compact-card"><PanelTitle panelId="case" title="Case Information"><ReviewBadge status={review.status ?? "pendiente"} /></PanelTitle>{panelVisible("case") ? <dl className="info-list compact-info"><div><dt>Case ID</dt><dd>{displayRun.caseId ?? studyReview?.caseId}</dd></div><div><dt>Subject Ref</dt><dd>{selectedDetail?.study?.patientId ?? studyReview?.patientId ?? "PAT-0087"}</dd></div><div><dt>Study Date</dt><dd>{selectedDetail?.study?.studyDate ?? studyReview?.studyDate ?? "2026-07-01"}</dd></div><div><dt>Plane</dt><dd>{currentSeries?.plane ?? displayRun.plane}</dd></div><div><dt>Model</dt><dd>{displayRun.modelKey}</dd></div><div><dt>Run ID</dt><dd>{displayRun.runId}</dd></div></dl> : hiddenPlaceholder}</article>
          <article className="panel-card compact-card"><PanelTitle panelId="ai-output" title="AI Output"><StatusBadge tone={outputTone(aiOutput.status)}>{aiOutput.status ?? "ai_output_pending"}</StatusBadge></PanelTitle>{panelVisible("ai-output") ? <><strong>{aiOutput.label ?? "Study review contract ready"}</strong><p className="muted compact-copy">{aiOutput.description ?? "Workspace visual con datos de-identificados."}</p></> : hiddenPlaceholder}</article>
          <article className="panel-card compact-card"><PanelTitle panelId="series" title="Series" />{panelVisible("series") ? <div className="series-list compact-list">{seriesList.map((item: any) => <button className={`series-item ${currentSeries?.id === item.id ? "active" : ""}`} key={item.id} onClick={() => selectSeries(item)} type="button"><span className="thumbnail" /><span><strong>{item.name}</strong><small>{item.plane} · {item.sliceCount} slices</small></span></button>)}</div> : hiddenPlaceholder}</article>
          <article className="panel-card compact-card"><PanelTitle panelId="masks" title="Masks" />{panelVisible("masks") ? <div className="mask-list compact-mask-list">{masks.map((mask: any) => { const visible = maskVisibility[mask.id] ?? mask.enabled ?? true; return <label className={selectedMask === mask.id ? "selected" : ""} key={mask.id}><input checked={visible} onChange={() => toggleMask(mask.id)} type="checkbox" /><i style={{ background: mask.color }} /><button onClick={() => setSelectedMask(mask.id)} type="button">{mask.label}</button><small>{visible ? "on" : "off"}</small></label>; })}</div> : hiddenPlaceholder}</article>
        </aside>
        ...
        <section className="center-column">
          ...
          <div className="toolbar compact-toolbar"><button className={editMode ? "active" : ""} onClick={() => setEditMode((value) => !value)} type="button">Edit Mask</button><button onClick={() => setSelectedLandmark("L4-L5")} type="button">Add Landmark</button><button type="button">Recalculate</button><button className={overlayEnabled ? "active" : ""} onClick={() => setOverlayEnabled((value) => !value)} type="button">Overlay</button><label className="opacity-control">Opacity <input min="25" max="100" value={overlayOpacity} onChange={(event) => setOverlayOpacity(Number(event.target.value))} type="range" /></label></div>
          ...
          {tab === "3D Reconstruction" ? <article className="panel-card full-viewer"><SpineReconstructionPreview /></article> : <div className="viewer-stack compact-viewer-stack"><MriSliceViewer variant={currentSeries?.plane === "axial" ? "axial" : "sagittal"} series={currentSeries} masks={masks} landmarks={landmarks} maskVisibility={maskVisibility} selectedMask={selectedMask} sliceIndex={activeSlice} onSliceChange={(slice) => currentSeries && setSliceBySeries((current) => ({ ...current, [currentSeries.id]: slice }))} overlayEnabled={overlayEnabled} overlayOpacity={overlayOpacity / 100} editMode={editMode} selectedLandmark={selectedLandmark} onSelectMask={setSelectedMask} onSelectLandmark={setSelectedLandmark} /><article className="panel-card compact-card legend-card"><PanelTitle panelId="legend" title="Legend" />{panelVisible("legend") ? <div className="legend-grid">{masks.map((mask: any) => <span key={mask.id}><i style={{ background: mask.color }} />{mask.label}</span>)}</div> : hiddenPlaceholder}</article></div>}
        </section>

        <aside className="right-column">
          <section className="panel-card results-panel"><PanelTitle panelId="results" title="Resultados IA y revisión"><StatusBadge tone={hasReviewerDrafts ? "amber" : "blue"}>{hasReviewerDrafts ? "cambios pendientes" : "editable"}</StatusBadge></PanelTitle>{panelVisible("results") ? <><p className="muted compact-copy">Vista unica de mediciones: valor IA, ajuste del medico y estado. Se persiste solo al guardar.</p><div className="comparison-table unified-results-table"><div className="comparison-head"><span>Medición</span><span>IA</span><span>Reviewer</span><span>Estado</span></div>{studyMeasurements.map((item: any) => { const persistedReviewerValue = getPersistedReviewerValue(item.id); const draftValue = reviewerValues[item.id]; const inputValue = draftValue ?? item.reviewerValue ?? persistedReviewerValue ?? ""; const status = draftValue !== undefined && draftValue !== "" ? "draft" : persistedReviewerValue ? "guardado" : item.status ?? "pendiente"; return <div className="comparison-row compact-comparison-row" key={item.id}><span><strong>{item.label}</strong><small>{item.level ?? "L4-L5"} · {Math.round((item.confidence ?? 0) * 100)}%</small></span><span>{item.aiValue ?? item.value} {item.unit}</span><input value={inputValue} onChange={(event) => updateReviewerValue(item, event.target.value)} placeholder="sin cambios" /><span>{status}</span></div>; })}</div><div className="review-actions compact-actions"><button className="primary-button" disabled={!hasReviewerDrafts || saving} onClick={commitReviewerMeasurements} type="button">Guardar medidas</button><button className="ghost-button" disabled={!hasReviewerDrafts || saving} onClick={() => setReviewerValues({})} type="button">Descartar</button>{hasReviewerDrafts && <span className="muted">{reviewerDraftCount} cambio/s</span>}</div></> : hiddenPlaceholder}</section>
          <AgentSummary agentDecision={studyReview?.aiOutput?.agentDecision ?? run.agentDecision} />
          <section className="panel-card notes-card compact-card"><PanelTitle panelId="decision" title="Notas y decisión" />{panelVisible("decision") ? <><textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Observaciones profesionales de revision..." /><div className="review-actions compact-actions"><select value={reviewStatus} onChange={(event) => setReviewStatus(event.target.value as ReviewStatus)}><option value="pendiente">pendiente</option><option value="aceptado">aceptado</option><option value="observado">observado</option><option value="descartado">descartado</option></select><button className="ghost-button" disabled={saving} onClick={() => void save(reviewStatus)} type="button">Save Draft</button><button className="primary-button" disabled={saving} onClick={() => void save("aceptado")} type="button">Approve</button><button className="warning-button" disabled={saving} onClick={() => void save("observado")} type="button">Observe</button></div></> : hiddenPlaceholder}</section>
